...
export default function EditProgrammingLanguage() {
  ...
  const loadLanguage = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Failed to load programming language');
      }
    } catch (err) {
      console.error('Load error:', err);
      setError('An error occurred while loading the programming language');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.error || 'Failed to update programming language');
      }
    } catch (err) {
      console.error('Submit error:', err);
      setError('An error occurred while updating the programming language');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (languageLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="text-center">
        <div className="w-16 h-16 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin mx-auto"></div>
        <p className="mt-4 text-lg font-medium text-gray-700 dark:text-gray-300">Loading programming language...</p>
      </div>
    </div>;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold mb-2 text-gradient">Edit Programming Language</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Update this programming language in your portfolio
            </p>
          </div>
